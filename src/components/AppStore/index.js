...
class AppStore extends Component {
  ...
  render() {
    ...
    const searchResult = this.updateSearch()
        ...
        <ul className="tab-item">
          {tabsList.map(eachTabDetails => (
            <TabItem
              ...
              isActive={activeTabId === eachTabDetails.tabId}
            />
          ))}
        </ul>

        <ul className="app-images">
          {filteredApps.map(eachImage => (
            <AppItem key={eachImage.appId} appsDetails={eachImage} />
          ))}
        </ul>
      </div>
    )
